...
class BaseOpenaiProcessor:
    ...
    def set_up_llm(self):
        ...
        load_dotenv()
        llm = OpenAI(
            api_key=os.getenv("OPENAI_API_KEY", "sk-5e39540640374287898aa19f80d8d3d9"),
    ...
    def send_message(
        ...
        **kwargs
        ...
        if os.getenv("USE_MOCK_LLM", "false").lower() == "true":
            print("[MOCK] 使用模拟 DashScope 响应")
            # 必须设置 response_data 属性
            self.response_data = {"model": "mock", "input_tokens": 0, "output_tokens": 0}
            if is_structured and response_format is not None:
                return {
                    "step_by_step_analysis": "这是模拟的逐步推理过程。",
                    "reasoning_summary": "模拟摘要",
                    "relevant_pages": [1],
                    "final_answer": "42"
                }
            else:
